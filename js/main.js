...
const getNewArray = (arr, arrLength) => {
  const newArray = [];
  for (let i = 0; i < arrLength; i++) {
    newArray.push(arr[getRandomIntegerInRange(0, arr.length - 1)]);
  }
  return newArray;
};

const getRandomObject = function (arrLength) {
  const objectsArray = [];
  for (let i = 0; i < arrLength; i++) {
    const someObject = {
      author: {
      ...
      offer: {
        ...
        features: getRandomIntegerInRange(0, FEATURES_ARRAY.length),
...
const cardObjects = getRandomObject(8);
...
const renderCard = function (object) {
  ...
  // cardElement.querySelector('.popup__photos').style.fill = object.offer.photos;
